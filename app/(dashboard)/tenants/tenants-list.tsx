...
export function TenantsList({ initialTenants, allProperties }: TenantsListProps) {
      ...
      <div className="grid gap-6">
        {filteredTenants.length === 0 ? (
          <div className="text-center py-20 glass rounded-[2.5rem] border-dashed border-2 border-slate-200">
            ...
            <p className="text-slate-500 font-bold">No tenants found.</p>
            <button 
              onClick={() => {setSearchTerm(""); setIsAddModalOpen(true);}}
              className="mt-4 text-primary font-bold hover:underline"
            >
              Clear search or add new tenant
            </button>
          </div>
        ) : (
